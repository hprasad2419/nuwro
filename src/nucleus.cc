...
nucleus::nucleus(params &par)
    : p(par.nucleus_p), n(par.nucleus_n), spectator(NULL), d(NULL), i(NULL),
      _Eb(par.nucleus_E_b * MeV), _kf(par.nucleus_kf * MeV),
...
//, sf_on(par.sf_method)
{
  ...
  if (p > 54) { // Calcium grid
    W00.Attach_ptr(Pb208_W00pp, Pb208_W00np, Pb208_W00pn, Pb208_W00_3p3h);
    W03.Attach_ptr(Pb208_W03pp, Pb208_W03np, Pb208_W03pn, Pb208_W03_3p3h);
    W11.Attach_ptr(Pb208_W11pp, Pb208_W11np, Pb208_W11pn, Pb208_W11_3p3h);
    W12.Attach_ptr(Pb208_W12pp, Pb208_W12np, Pb208_W12pn, Pb208_W12_3p3h);
    W33.Attach_ptr(Pb208_W33pp, Pb208_W33np, Pb208_W33pn, Pb208_W33_3p3h);
  } else if ((p > 12) and (p < 54)) { // Calcium grid
...
particle nucleus::get_nucleon(vec r) {
  ...
  switch (kMomDist) {
  case 1:
    p0.set_momentum(rand_from_ball(_kf));
    break; // global fermi gas
  case 2:
    p0.set_momentum(rand_from_ball(localkf(p0)));
    break; // local fermi gas
  case 3:
    p0.set_momentum(bodek_rand_from_ball(_kf));
    break; // Bodek Ritchie
  case 4:
    assert("effective SF can be run only for carbon and oxygen" && p == n &&
           (p == 6 || p == 8)); // effective SF is defined only for two targets
  ...
  case 6:
    p0.set_momentum(rand_from_ball(localkf(p0)));
    break; // effective potential
  default:
    p0.set_momentum(rand_from_ball(localkf(p0))); // local Fermi Gas
  }

  // account for the fact that the nucleus is in motion
  // p0.set_momentum(p0.p()+vec(_p4)/Ar()); ////???????
  if (not(p0.v2() < 1)) {
    cerr << kMomDist << " p=" << this->p << " n=" << this->n
         << " lkf=" << localkf(p0) << " v2=" << p0.v2() << " " << p0.v()
         << endl;
    // assert(p0.v2()<1);
  }
  return p0;
