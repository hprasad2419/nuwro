...
// Lead hadronic grid
#include "MEC_data/Pb208/Pb208_3p3h.h"
#include "MEC_data/Pb208/Pb208_np.h"
#include "MEC_data/Pb208/Pb208_pn.h"
#include "MEC_data/Pb208/Pb208_pp.h"

using namespace std;

// A class to load Valencia 2020 model hadronic tables Phys. Rev. C 102 (2020)
// 024601 J.E. Sobczyk Class Implemented by Hemant Prasad on 23 / 11 / 2024
...
/// proton and neutron density as well as local fermi momentum and Pauli
/// blocking the number of nucleons can change as a result of scattering and the
/// density and Fermi momentum should are consistently updated - this is still
/// NOT implemented
...
public:
  ...
  double cosine_threshold =
      0.01; ///< angular threshold to identify noninteracted struck nucleon

private:
  nucleus_data *d; ///< density profiles from experimental data

public:
  isotope *i; ///< isotope data from: "The Ame2003 atomic mass evaluation (II)"
              ///< by G.Audi, A.H.Wapstra and C.Thibault Nuclear Physics A729 p.
              ///< 337-676, December 22, 2003.
  ...
  int kMomDist; ///< Type of nucleon momentum distribution:
                ...
                /// 4 - effective spectral function (for carbon and oxygen, else
                /// lFG); 5 - deuterium (forced for H2); 6 - effective potential
...
public:
  ...
  double localkf(particle &pa); ///< local Fermi momentum for particle (pdg and
                                ///< position dependent)
  double localkf_(int pdg, double r); ///< local Fermi momentum from pdg code
                                      ///< and  dist r from nucleus center
  ...
  double Eb() {
    return _Eb;
  } ///< nucleon binding energy (from experimantal data)
  particle
  get_nucleon(vec r); ///< random nucleon located at r (used in Interaction.cc)
  particle get_nucleon();            ///< random nucleon (used in nuwro.cc)
  bool pauli_blocking(particle &pa); ///< true if the particle is blocked
  bool pauli_blocking(
      particle p[],
      int n); ///< true if any of p[0]..p[n-1] is blocked (used in cascade)
  bool pauli_blocking_old(particle &pa,
                          double p0); ///< TRUE = PARTICLE IS BLOCKED (p0 -
...
////////////////////////////////////////////////////////////////////////
inline double
nucleus::localkf(particle &pa) ///< local Fermi momentum for particle (pdg and
                               ///< position dependent)
{
...
///////////////////////////////////////////////////////////////////////
inline bool nucleus::pauli_blocking_old(particle &pa,
                                        double p0) // TRUE = PARTICLE IS BLOCKED
{
  ...
  case 0:
    return false;
  case 2:
    return pa.momentum() < localkf(pa); // Local Fermi Gas
  case 4:
    return pa.momentum() < localkf(pa); // Local Fermi Gas
  case 5:
    return pa.momentum() < p0; // deuteron
  default:
    return pa.momentum() < _kf; // Global Fermi Gas
  }
}
...
inline double nucleus::hyp_BE(double r, int pdg) {
  ...
  case 3112:
  case 3222:
    return Sigma_Eb * density(r) / density(0);
  default:
    std::cout << "This is not a hyperon! Returning 0 for hyperon potential"
              << std::endl;
    return 0;
  }
